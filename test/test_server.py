...
class TestServerInitialization(unittest.TestCase):
    ...
    def test_config_singleton_initialized(
        ...
        mock_config.CUSTOMER_API_PORT = 8184
...
class TestAppConfiguration(unittest.TestCase):
    ...
    def test_subscription_routes_not_registered(self):
        """Test that /api/subscription routes are not registered."""
        response = self.client.get("/api/subscription")
        self.assertEqual(response.status_code, 404)

    def test_dashboard_routes_not_registered(self):
        """Test that /api/dashboard routes are not registered."""
        response = self.client.get("/api/dashboard")
        self.assertEqual(response.status_code, 404)

    def test_card_routes_not_registered(self):
        """Test that /api/card routes are not registered."""
    ...
    def test_all_blueprints_registered(self):
        ...
        # Check that doomed blueprints are absent
        self.assertNotIn("subscription_routes", blueprint_names)
        self.assertNotIn("dashboard_routes", blueprint_names)
        self.assertNotIn("card_routes", blueprint_names)

        # Check that active domain blueprints are registered
    ...
    def test_url_map_contains_expected_routes(self):
        ...
        self.assertFalse(any("/api/subscription" in rule for rule in rules))
        self.assertFalse(any("/api/dashboard" in rule for rule in rules))
        self.assertFalse(any("/api/card" in rule for rule in rules))
...
class TestServerExecution(unittest.TestCase):
    ...
    def test_main_execution_uses_config_port(self, mock_config, mock_run):
        """Test that __main__ execution uses CUSTOMER_API_PORT from config."""
        # Arrange
        mock_config.CUSTOMER_API_PORT = 9999
        ...
        if hasattr(server_module, "__name__"):
            ...
            api_port = mock_config.CUSTOMER_API_PORT
