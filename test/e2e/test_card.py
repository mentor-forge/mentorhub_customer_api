"""
E2E tests for Card endpoints.

These tests verify that Card endpoints work correctly by making
...
def test_create_card_endpoint():
    """Test POST /api/card endpoint and verify record persists in database."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-card",
        "description": "E2E test card document",
    }

    response = requests.post(f"{BASE_URL}/api/card", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-card"
...
def test_get_cards_with_name_filter():
    ...
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/card?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_card_endpoints_require_auth():
    ...
    response = requests.get(f"{BASE_URL}/api/card")
