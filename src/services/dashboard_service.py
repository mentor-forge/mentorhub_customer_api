"""
Dashboard service for business logic and RBAC.

Handles RBAC checks and MongoDB operations for Dashboard domain.
"""
...
class DashboardService:
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update dashboard documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create dashboard documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_dashboard(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the created dashboard document
        """
        try:
            DashboardService._check_permission(token, 'create')
            ...
            dashboard_id = mongo.create_document(config.DASHBOARD_COLLECTION_NAME, data)
            logger.info(f"Created dashboard { dashboard_id} for user {token.get('user_id')}")
            return dashboard_id
        except HTTPForbidden:
            raise
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error creating dashboard: {error_msg}")
            raise HTTPInternalServerError(f"Failed to create dashboard: {error_msg}")
    
    @staticmethod
    def get_dashboards(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
        Get infinite scroll batch of sorted, filtered dashboard documents.
        ...
        """
        try:
            DashboardService._check_permission(token, 'read')
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.DASHBOARD_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} dashboards (has_more={result['has_more']}) "
            ...
            raise HTTPInternalServerError("Failed to retrieve dashboards")
    
    @staticmethod
    def get_dashboard(dashboard_id, token, breadcrumb):
        """
        Retrieve a specific dashboard document by ID.
        ...
        Returns:
            dict: The dashboard document
            
        Raises:
            HTTPNotFound: If dashboard is not found
        """
        try:
            DashboardService._check_permission(token, 'read')
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            dashboard = mongo.get_document(config.DASHBOARD_COLLECTION_NAME, dashboard_id)
            if dashboard is None:
                raise HTTPNotFound(f"Dashboard { dashboard_id} not found")
            
            logger.info(f"Retrieved dashboard { dashboard_id} for user {token.get('user_id')}")
            return dashboard
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving dashboard { dashboard_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to retrieve dashboard { dashboard_id}")
    
    @staticmethod
    def update_dashboard(dashboard_id, data, token, breadcrumb):
        """
        Update a dashboard document.
        ...
        Returns:
            dict: The updated dashboard document
            
        Raises:
            HTTPNotFound: If dashboard is not found
        """
        try:
            DashboardService._check_permission(token, 'update')
            DashboardService._validate_update_data(data)
            ...
            updated = mongo.update_document(
                config.DASHBOARD_COLLECTION_NAME,
            ...
            logger.info(f"Updated dashboard { dashboard_id} for user {token.get('user_id')}")
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating dashboard { dashboard_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to update dashboard { dashboard_id}")