...
class CustomerService:
    ...
    @staticmethod
    def get_customers(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
    ...
    def get_customer(customer_id, token, breadcrumb):
        ...
        try:
            ...
            logger.info(f"Retrieved customer { customer_id} for user {token.get('user_id')}")
            return customer
        except HTTPNotFound:
            raise
        except Exception as e:
