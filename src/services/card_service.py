"""
Card service for business logic and RBAC.

Handles RBAC checks and MongoDB operations for Card domain.
"""
...
class CardService:
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update card documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create card documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_card(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the created card document
        """
        try:
            CardService._check_permission(token, 'create')
            ...
            card_id = mongo.create_document(config.CARD_COLLECTION_NAME, data)
            logger.info(f"Created card { card_id} for user {token.get('user_id')}")
            return card_id
        except HTTPForbidden:
            raise
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error creating card: {error_msg}")
            raise HTTPInternalServerError(f"Failed to create card: {error_msg}")
    
    @staticmethod
    def get_cards(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
        Get infinite scroll batch of sorted, filtered card documents.
        ...
        """
        try:
            CardService._check_permission(token, 'read')
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.CARD_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} cards (has_more={result['has_more']}) "
            ...
            raise HTTPInternalServerError("Failed to retrieve cards")
    
    @staticmethod
    def get_card(card_id, token, breadcrumb):
        """
        Retrieve a specific card document by ID.
        ...
        Returns:
            dict: The card document
            
        Raises:
            HTTPNotFound: If card is not found
        """
        try:
            CardService._check_permission(token, 'read')
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            card = mongo.get_document(config.CARD_COLLECTION_NAME, card_id)
            if card is None:
                raise HTTPNotFound(f"Card { card_id} not found")
            
            logger.info(f"Retrieved card { card_id} for user {token.get('user_id')}")
            return card
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving card { card_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to retrieve card { card_id}")
    
    @staticmethod
    def update_card(card_id, data, token, breadcrumb):
        """
        Update a card document.
        ...
        Returns:
            dict: The updated card document
            
        Raises:
            HTTPNotFound: If card is not found
        """
        try:
            CardService._check_permission(token, 'update')
            CardService._validate_update_data(data)
            ...
            logger.info(f"Updated card { card_id} for user {token.get('user_id')}")
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating card { card_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to update card { card_id}")