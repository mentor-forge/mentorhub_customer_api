"""
Subscription service for business logic and RBAC.

Handles RBAC checks and MongoDB operations for Subscription domain.
"""
...
# Allowed sort fields for Subscription domain
...
class SubscriptionService:
    ...
    - Business logic for Subscription domain
    """
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update subscription documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create subscription documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_subscription(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the created subscription document
        """
        try:
            SubscriptionService._check_permission(token, 'create')
            ...
            subscription_id = mongo.create_document(config.SUBSCRIPTION_COLLECTION_NAME, data)
            logger.info(f"Created subscription { subscription_id} for user {token.get('user_id')}")
            return subscription_id
        except HTTPForbidden:
            raise
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error creating subscription: {error_msg}")
            raise HTTPInternalServerError(f"Failed to create subscription: {error_msg}")
    
    @staticmethod
    def get_subscriptions(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
        Get infinite scroll batch of sorted, filtered subscription documents.
        ...
        """
        try:
            SubscriptionService._check_permission(token, 'read')
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.SUBSCRIPTION_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} subscriptions (has_more={result['has_more']}) "
            ...
            raise HTTPInternalServerError("Failed to retrieve subscriptions")
    
    @staticmethod
    def get_subscription(subscription_id, token, breadcrumb):
        """
        Retrieve a specific subscription document by ID.
        ...
        Returns:
            dict: The subscription document
            
        Raises:
            HTTPNotFound: If subscription is not found
        """
        try:
            SubscriptionService._check_permission(token, 'read')
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            subscription = mongo.get_document(config.SUBSCRIPTION_COLLECTION_NAME, subscription_id)
            if subscription is None:
                raise HTTPNotFound(f"Subscription { subscription_id} not found")
            
            logger.info(f"Retrieved subscription { subscription_id} for user {token.get('user_id')}")
            return subscription
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving subscription { subscription_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to retrieve subscription { subscription_id}")
    
    @staticmethod
    def update_subscription(subscription_id, data, token, breadcrumb):
        """
        ...
        Returns:
            dict: The updated subscription document
            
        Raises:
            HTTPNotFound: If subscription is not found
        """
        try:
            SubscriptionService._check_permission(token, 'update')
            SubscriptionService._validate_update_data(data)
            ...
            updated = mongo.update_document(
                config.SUBSCRIPTION_COLLECTION_NAME,
            ...
            logger.info(f"Updated subscription { subscription_id} for user {token.get('user_id')}")
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating subscription { subscription_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to update subscription { subscription_id}")