...
The Customer API is built with `api-utils==1.0.3` and implements the 1.0.0 list GET contract (`offset`/`size` request headers, JSON array responses). The token dict from `create_flask_token()` uses `display_name`.
